...
def encode_npy(img_filepath, mask_filepath):
...
def encode_img(img_filepath, mask_filepath):
...
def read_and_decode(filename_queue=None, img_dims=[512,512,3], size_of_batch=16,\
  ...
  if shuffle:
    image, mask, file_path= tf.train.shuffle_batch([image, mask, file_path],
                                         batch_size=size_of_batch,
                                         capacity=100 + 3 * size_of_batch,
                                         min_after_dequeue=100,
                                         num_threads=num_of_threads)
  else:
    image, mask, file_path = tf.train.batch([image, mask, file_path],
                                 batch_size=size_of_batch,
                                 capacity=100,
