...
def propagate(x, y, xml, slide, patch_dim, hit_threshold, whiteness_threshold, roi_corners, zoom, anno_name, data_dir):
    """
    Analyze potential benign image patch  
    ...
    if hit_count >= hit_threshold and not bad_crop:
...
def create_crops(svs,xml,anno_coordinates,anno_names,patch_dim,zoom,data_dir):
    """
    Goes through histopathology svs slide and xml annotations creating normal crops
    ...
    hit_threshold = (patch_dim*patch_dim*0.5)
    ...
    for anno in range(len(anno_coordinates)):
        ...
        for x in tqdm(range(num_width_iters)):
            for y in range(num_height_iters):
                ...
                propagate(current_width,current_height,xml,slide,patch_dim,hit_threshold,whiteness_threshold,roi_corners,zoom,anno_name,data_dir)
